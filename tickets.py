...
def get_tickets():
    ...
    if current_user['role'] == 'customer':
        ...
        tickets = Ticket.query.filter(Ticket.assigned_to.in_([None, current_user['id']])).all()
    else:
...
def update_ticket(ticket_id):
    ...
    data = request.get_json()
    if 'status' in data and current_user['role'] in ['tech', 'admin']:
        ticket.status = data['status']
        if data['status'] == 'resolved':
            ticket.resolved_at = datetime.utcnow()
    
    if 'assigned_to' in data and current_user['role'] in ['tech', 'admin']:
